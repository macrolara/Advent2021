{
    ...
    rl.on('line', (line: string) => {
        ...
        let points: Map<number, {yv: number, yval: number, xv: number | undefined, steps: number, maxY: number}> = new Map();
        let maxSteps = 0;
        for (let v = 0; v <= yBox.min * -1; v++) {
            ...
            do {
                ...
                y += velocity; velocity--; step++; maxY = Math.max(maxY, y);
            } while (y >= yBox.min);
        }
        ...
        for (let v = startX; v <= xBox.max; v++) {
            ...
            do {
                if (x >= xBox.min) {
                    if (points.has(step)) points.get(step)!.xv = v;
                }
                x += velocity;
        ...
        console.log(JSON.stringify([...points.values()].filter(p => p.xv !== undefined).reduce((pv, cv) => {
            if (cv.maxY > pv.maxY) return {xv: cv.xv!, yv: cv.yv, maxY: cv.maxY};
            return pv;}, {xv: 0, yv: 0, maxY: 0})));
    });
}