    ...
    rl.on('close', function () {
        ...
        while (expressions_1.length > 0) {
            if (expression === undefined)
                expression = expressions_1.shift();
            else {
                expression = new PairExpression_1({ line: "", offset: 0 }, [expression, expressions_1.shift()]);
                do {
                    // console.log("S " + expression.toString())
                    while (expression.explode({ left: undefined, right: undefined, remove: false }, 1))
                        ; //console.log("E " + expression.toString());
                } while (expression.split());
                console.log(expression.toString() + " - " + (expression === null || expression === void 0 ? void 0 : expression.getMagnitude()));
            }
        }
    });
}
