{
    ...
    rl.on('close', () => {
        let expression: PairExpression | undefined;
        while (expressions.length > 0) {
            if (expression === undefined) expression = expressions.shift();
            else {
                expression = new PairExpression({line: "", offset: 0}, [expression, expressions.shift()!])
                do {
                    // console.log("S " + expression.toString())
                    while (expression!.explode({left: undefined, right: undefined, remove: false}, 1)); //console.log("E " + expression.toString());
                } while (expression!.split());
                console.log(expression!.toString() + " - " + expression?.getMagnitude());
            }
        }
    });
}