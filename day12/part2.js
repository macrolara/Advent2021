...
};
{
    ...
    function getPaths2(nodeName, visited, hasRevisited) {
        var node = nodes_1.get(nodeName);
        var count = 0;
        if (visited.includes(node.name) && !node.canRevisit) {
            if (hasRevisited || node.name === "start")
                return count;
            hasRevisited = true;
        }
        ...
        if (node.neighbours.includes("end"))
            count++;
        node.neighbours.filter(function (n) { return n !== "end"; }).forEach(function (n) { return count += getPaths2(n, __spreadArray([], visited, true), hasRevisited); });
        return count;
    }
